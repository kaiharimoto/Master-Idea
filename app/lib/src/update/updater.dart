...
class Updater extends ChangeNotifier {
  ...
  ///
  /// Silent by design: a failure here means the device is offline or GitHub is
  /// rate-limiting, neither of which is worth interrupting someone who opened
  /// the app to put an idea before the council.
...
class _RealTransport implements UpdateTransport {
  ...
  // One channel for the app's whole native surface, not one per feature:
  // installing, sharing and saving are all plumbing of the same kind.
  ...
  static String get _agent =>
      'MasterIdea/${BuildInfo.version}+${BuildInfo.build} '
      '(${BuildInfo.platform})';
}
