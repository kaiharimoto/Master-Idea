...
/// of that is here, so it can be proven against captured GitHub payloads
/// rather than only against GitHub.
library;

/// Which artefact a platform can actually install.
enum UpdatePlatform {
  /// An APK, handed to the system package installer.
  android,

  /// A setup program, run silently, or a portable zip extracted by hand.
  windows,
...
enum AssetKind {
  ...
  /// A Windows setup program: run silently, then relaunched.
  installer,

  /// A Windows portable zip, which the user extracts by hand. A running
  /// executable cannot be replaced underneath itself.
  archive,
}
...
class ReleaseAsset {
  ...
  /// `0.1.0+57 · a1b2c3d`, which reads the same way `BuildInfo.label` does, so
  /// the two can be compared by eye without translating between them.
...
/// because "update failed" with no reason is what makes an updater feel broken
...
enum UpdateOutcome {
  ...
  /// The running build has no number to compare against — built locally
  /// rather than by CI.
  unknownBuild,
...
class UpdateCheck {
  ...
  /// even for [UpdateOutcome.upToDate], so the UI can always say what the
  /// latest build is.
...
/// `MasterIdea-57-a1b2c3d.apk`
final RegExp _apk = RegExp(r'^MasterIdea-(\d+)-([0-9a-f]{7,40})\.apk$');

/// `MasterIdeaSetup-57-a1b2c3d.exe`
final RegExp _setup = RegExp(r'^MasterIdeaSetup-(\d+)-([0-9a-f]{7,40})\.exe$');

/// `MasterIdea-windows-x64-57-a1b2c3d.zip`
final RegExp _zip = RegExp(
  r'^MasterIdea-windows-x64-(\d+)-([0-9a-f]{7,40})\.zip$',
);
...
UpdateCheck readRelease(
  ...
  // Windows publishes two shapes. The installer is preferred wherever both
  // are present, because it is the one that can update itself without the
